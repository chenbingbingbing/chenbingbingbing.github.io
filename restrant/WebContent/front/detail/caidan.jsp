		...
		<div id="menu">
							...
							<dl>
								<dt>
									<a href="<%=contextPath %>/frontController?method=getFoodDetail&id=${l.id}">
										<img width="214px" height="145px" src="<%=contextPath %>/${l.img}" />
									</a>
								</dt>
								<dd class="f1">
			...
			<div id="foot">
				...
				<span style="float:left; line-height:53PX; margin-left:-50px; font-weight:bold; ">
					<a href="<%=contextPath %>/frontController?method=getFoodList&page=${pageUtil.prevPage}" style="font-weight:bold">&lt;&lt;</a>
				</span>									
				<div id="btn">
					<ul>
						<!-- 参看 百度, 谷歌是 左 5 右 4 -->
					<% 
						...
						if(pageUtil!=null){
						for(int i=1;i<=pageUtil.getLastPage();i++){
					%>					
						<li><a href="<%=contextPath %>/frontController?method=getFoodList&page=<%=i%>"><%=i %></a></li>										
					<%	
						}
						}
					%>							
				...
				<span style="float:right; line-height:53px; margin-right:10px;  ">
					<a href="<%=contextPath %>/frontController?method=getFoodList&page=${pageUtil.nextPage}" style=" text-decoration:none;color:#000000; font-weight:bold">&gt;&gt;</a>
				</span>													
			</div>
